...
function save_su_credentials($su_username, $su_password){
  $su_db = new cyz_json_db('su');

  // Set Table Name
  $table_name = 'SU';

  // Create Table
  $su_db->update_table($table_name);

  // get all super user data
  $sus = $su_db->get_rows($table_name);

  foreach($sus as $key => $su_data){
  ...
  if($su_db->table_exists($table_name)){
    $updated = $su_db->update_row($table_name, null, $new_user);
  }
