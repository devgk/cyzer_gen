...
/** Include form fields */
require_once(CYZ_LIB.'/form/fields.php');

/** Include session token */
require_once(CYZ_LIB.'/security/session-token.php');
...
/** Include database connect script to make DB connection */
require_once(CYZ_ADMIN_CTRL.'/setup/functions/db/connect.php');

/** Include database config-constructor script to setup permanent
 *  Database connection using configuration file "app-config" */
require_once(CYZ_ADMIN_CTRL.'/setup/functions/db/config-constructor.php');
...
if(verify_session_token($form, $form_action)):
  ...
  if(cyz_db_test::is_connected()){
    $config_file = cyz_setup_db_config();
    ...
    array_push($form_error, 'Unable to connect! Please check database details');
  }


  /** After testing connection and config file setup
   *  Disconnect the database */
  cyz_db_test::disconnect();
